...
def generate_dashboard():
    ...
    for completion in reversed(completions):
        ...
        # Tries "title", falls back to "challenge", then "id", or empty string
        title = completion.get("title") or completion.get("challenge") or completion.get("id") or ""
    ...
    if not completion_rows:
        ...
        # Converts list of row strings into a single clean string of HTML
        table_body = "".join(completion_rows)
...
def calculate_streak(completions):
    ...
    for index in range(1, len(dates)):
        difference = (dates[index] - dates[index - 1]).days
        if difference == 1:
            streak += 1
        else:
            streak = 1
...
def generate_dashboard():
    ...
    for technology, challenge_list in challenges.items():
        ...
        percent = (
            round((completed_for_technology / total) * 100)
            if total
            else 0
        )
        ...
        <div class="tech-card">
            ...
            <p>
                {completed_for_technology} / {total} challenges completed
            </p>
        </div>
        """
    # --------------------------------------------------
    ...
    challenge_lookup = {}
    for tech_list in challenges.values():
        for ch in tech_list:
            if "id" in ch and "title" in ch:
                challenge_lookup[ch["id"]] = ch["title"]
    ...
    for completion in reversed(completions):
        ...
        <tr>
            <td>{completion.get("date", "")}</td>
            <td>
                {technology_name(completion.get("technology", ""))}
            </td>
            <td>
                {title}
            </td>
    ...
    if not completion_rows:
        ...
        completion_table_body = "".join(completion_rows)
    ...
    if today_challenge:
        ...
        <section class="today-challenge">
            ...
            <div class="today-meta">
                <span>📅 {today_challenge["date"]}</span>
                <span>🛠️ {today_challenge["technology"]}</span>
                <span>🆔 {today_challenge["challenge_id"]}</span>
            </div>
            <p class="objective">
                {today_challenge["objective"]}
            </p>
        ...
        <section class="today-challenge">
            ...
            <p>No challenge has been generated for today.</p>
        </section>
        """
...
<div class="container">
    ...
    <section class="hero">
        <h1>🚀 Developer Learning Lab</h1>
        <p>Personal Developer Learning Dashboard</p>
    </section>
    ...
    <section class="section">
        ...
        <div class="stats">
            ...
            <div class="stat">
                <h2>🏆 {streak["longest"]}</h2>
                <p>Longest Streak</p>
            </div>
            <div class="stat">
                <h2>📅 {streak["learning_days"]}</h2>
                <p>Learning Days</p>
            </div>
        </div>
    </section>
